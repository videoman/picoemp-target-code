...
void setup() {
    ...
    Serial.print("Setup complete on Feather.\n");
...
void loop()
{
    if (glitched == 1) {
        ...
        Serial.print("Starting For Loop on feather\n");
        cnt = 0;
        ...
        Serial.print("Glitch Check on feather\n");
        if (i != INNER_LOOP_CNT || j != OUTER_LOOP_CNT || cnt != INNER_LOOP_CNT * OUTER_LOOP_CNT) {
            Serial.print ("----------------------------Done Glitched on feather!\n");
            glitched = 1;
        }
    }
}
